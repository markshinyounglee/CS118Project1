...
int main(int argc, char **argv) {
  ...
  uint32_t received_ack = 0; // dealing with received packets
  uint32_t ack = 0;
  ...
  */
  ...
  while (1) {
    ...
    if(bytes_recvd > 0)
    {
      ...
      for (list<packet>::iterator iter = rcvbuf.bufcontent.begin(); iter != rcvbuf.bufcontent.end(); ++iter)
      {
        ...
        if(ack == iter->ack)
        {
          ...
          int length = rcvbuf.len;
          memcpy(payload, &writebuf[writebuflen], length);
          writebuflen += length;
          rcvbuf.len -= iter->length;
    ...
    else if(bytes_read == 0 || sndbuf.len >= MAX_WINDOW)
    {
    ...
    if(ack_recvd) // reset the timer if we receive a new ACK
    {
      start_clock = end_clock = clock();
    }
    // if 3 duplicate ACKs, resend the packet with the lowest sequence number
    uint32_t prev_ack = rcvbuf.bufcontent.back().ack;
    if (ack == prev_ack)
      ack_counter++;
    if (ack_counter) 
    {
