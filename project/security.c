...
void derive_keys() {
    ...
    params[1] = OSSL_PARAM_construct_octet_string("key", secret, (size_t) SECRET_SIZE);
...
int verify(char* data, size_t size, char* signature, size_t sig_size, EVP_PKEY* authority) {
    ...
    result = EVP_DigestVerifyFinal(mdctx, (unsigned char*) signature, sig_size);
...
size_t decrypt_cipher(char *cipher, size_t size, char *iv, char *data, int using_mac) {
    ...
    EVP_DecryptUpdate(ctx, (unsigned char*) data, &data_size, (const unsigned char*) cipher, size);
    EVP_DecryptFinal_ex(ctx, (unsigned char*) data + data_size, &padding_size);
