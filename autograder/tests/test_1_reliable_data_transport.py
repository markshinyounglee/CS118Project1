...
class TestRDT(unittest.TestCase):
    ...
    def make_test(self, size: int, timeout: int, use_ascii: bool, ref_client: bool, ref_server: bool, drop_rate: float, reorder_rate: float, name: str) -> None:
        ...
        if use_ascii:
            file = b''.join([choice(ascii_letters).encode()
                            for _ in range(size)])
        else:
            file = randbytes(size)
        ...
        if ref_server:
            server_runner = ProcessRunner(
                f'/autograder/source/src/server {server_port}', file, name + "_refserver.out")
        else:
            server_runner = ProcessRunner(
                f'runuser -u student -- {test_0_compilation.dir}/server {server_port}', file, name + "_yourserver.out")

        if ref_client:
            client_runner = ProcessRunner(
                f'/autograder/source/src/client localhost {client_port}', file, name + "_refclient.out")
        else:
            client_runner = ProcessRunner(
                f'runuser -u student -- {test_0_compilation.dir}/client localhost {client_port}', file, name + "_yourclient.out")
        ...
        ProcessRunner.run_two_until_size_or_timeout(
            server_runner, client_runner, size, timeout)
        p_thread.terminate()
        ...
        if file != server_runner.stdout:
            ...
            if not ref_server and not ref_client:
                ...
                print(
                    f"We inputted {size} bytes in your client and we received {len(server_runner.stdout)} bytes ", end='')
            elif ref_server:
                ...
                print(
                    f"We sent {size} bytes and your server received {len(server_runner.stdout)} bytes ", end='')
            print(
                f'with a percent difference of {byte_diff(file, server_runner.stdout)}%')
        ...
        if file != client_runner.stdout:
            ...
            if not ref_server and not ref_client:
                ...
                print(
                    f"We inputted {size} bytes in your server and we received {len(client_runner.stdout)} bytes ", end='')
            elif ref_client:
                ...
                print(
                    f"We sent {size} bytes and your client received {len(client_runner.stdout)} bytes ", end='')
            print(
                f'with a percent difference of {byte_diff(file, client_runner.stdout)}%')
    ...
    def test_self_ascii(self):
        """Data Transport (Your Client <-> Your Server): Small, ASCII only file (2 KB)"""
    ...
    @hide_errors()
    def test_self(self):
        """Data Transport (Your Client <-> Your Server): Small file (2 KB)"""
        if test_0_compilation.failed:
            self.fail()
    ...
    def test_client_normal(self):
        """Data Transport (Your Client <-> Reference Server): Small file (2 KB)"""
        if test_0_compilation.failed:
            self.fail()
    ...
    def test_server_normal(self):
        """Data Transport (Reference Client <-> Your Server): Small file (2 KB)"""
        if test_0_compilation.failed:
            self.fail()
        ...
        self.make_test(FILE_SIZE, TIMEOUT, False, True, False,
